...
+
È possibile vedere che il metodo POST di `/storage-provider/file-shares` richiama internamente tutte le API richieste per ciascuna delle funzioni e crea l'oggetto. Ad esempio, richiama l' `/storage-provider/performance-service-levels/`API per l'assegnazione del livello di servizio prestazioni nella condivisione file.
