...
* 100 GB di spazio libero su disco allocato alla `/opt/netapp/data` directory, montata su un'unità LVM o su un disco locale separato collegato al sistema di destinazione


[NOTE]
====
Per le directory e `/var/log` montate separatamente `/opt`, assicurarsi che `/opt` abbia 15 GB e `/var/log` 16 GB di spazio libero. La `/tmp` directory deve avere almeno 10 GB di spazio libero.
...
https://www.netapp.com/pdf.html?item=/media/13504-tr4621.pdf["Report tecnico 4621: Guida alle Best practice di Unified Manager"^]
