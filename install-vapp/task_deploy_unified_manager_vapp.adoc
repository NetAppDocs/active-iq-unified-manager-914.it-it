...
[NOTE]
====
A partire da Unified Manager 9,8, VMware Tools è stato sostituito da Open VM Tools  `open-vm-tools`). Non è più necessario installare VMware Tools come parte dell'installazione poiché `open-vm-tools` è incluso nel pacchetto di installazione di Unified Manager.
