...
ID gruppo statistiche
 a| 
`*0*` - INDICATORE_PRESTAZIONI (impostazione predefinita)
...
Utilizzando la notazione con punti è possibile definire una struttura gerarchica di cartelle. Ad esempio, inserendo `*stats.performance.netapp*` le statistiche si trova in *Stats* > *performance* > *NetApp*.
...
. Quando richiesto, digitare `*y*` per verificare che le informazioni immesse siano corrette.
. Premere un tasto qualsiasi per tornare al menu connessione server esterno.
. Digitare `*x*` per uscire dalla console di manutenzione.
