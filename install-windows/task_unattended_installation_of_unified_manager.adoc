...
|===
+
L' `/qr`opzione attiva la modalità silenziosa con un'interfaccia utente ridotta. Viene visualizzata un'interfaccia utente di base che mostra l'avanzamento dell'installazione. Non vengono richiesti input. Se pacchetti di terze parti come JRE, MySQL e 7zip non sono preinstallati, si dovrebbe usare l' `/qr`opzione. L'installazione non riesce se l' `/qn`opzione viene utilizzata su un server in cui non sono installati pacchetti di terze parti.

+
L' `/qn`opzione attiva la modalità silenziosa senza interfaccia utente. Durante l'installazione non viene visualizzata alcuna interfaccia utente o dettagli. Non utilizzare l' `/qn`opzione quando i pacchetti di terze parti non sono installati.
