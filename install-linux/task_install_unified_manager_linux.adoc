...
* È necessario aver scaricato il file di Unified Manager `.zip` dal sito di assistenza NetApp sul sistema di destinazione.
* È necessario verificare l'integrità del file scaricato `.zip`.
...
+
Se si ricevono errori di dipendenza, è necessario aggiungere `--nodeps` l'opzione per disinstallare il componente.

. Passare alla directory in cui è stato scaricato il file di installazione `.zip` ed espandere il bundle Unified Manager:
+
`unzip ActiveIQUnifiedManager-_<version>_.zip`

+
I moduli richiesti `.rpm` per Unified Manager vengono decompressi nella directory di destinazione.
...
+
.. Sul sistema su cui si sta installando Unified Manager, visualizzare l'elenco dei pacchetti disponibili e non disponibili: +
...
+
Se l'installazione non riesce con l'errore GPG NOKEY , utilizzare `rpm --import` per importare le chiavi da un URL:
...
[NOTE]
====
Non tentare l'installazione utilizzando comandi alternativi (come `rpm -ivh`). Una corretta installazione di Unified Manager su un sistema Red Hat Enterprise Linux o CentOS richiede che tutti i file di Unified Manager e i file correlati siano installati in un ordine specifico in una struttura di directory specifica imposta automaticamente dal `yum install netapp-um<version>.x86_64.rpm` comando.
